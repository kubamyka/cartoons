  ...
  Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
    if (isSearchActive) {
      TextField(value = query, placeholder = {
        ...
        maxLines = 1, colors = TextFieldDefaults.colors(focusedContainerColor = Color.Transparent,
      ...
      Text(text = stringResource(id = R.string.search_cartoons), fontSize = 18.sp,
    ...
    IconButton(onClick = toggleSearchActive) {
      Icon(
        ...
        contentDescription = if (isSearchActive) stringResource(R.string.close) else stringResource(
          R.string.search),
        tint = MaterialTheme.colorScheme.onSurfaceVariant,
      )
    }
